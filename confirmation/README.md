...
```bash
confirmation$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
confirmation$ sam local start-api
confirmation$ curl http://localhost:3000/
```
...
```bash
confirmation$ sam logs -n HelloWorldFunction --stack-name confirmation --tail
```
...
```bash
aws cloudformation delete-stack --stack-name confirmation
```
